...
function renderDashboard() {
    ...
    // Daily Missions (if user is logged in)
    if (getAuthUser()) {
        const missionsContainer = document.createElement('div');
        missionsContainer.id = 'dashboard-missions';
        frame.appendChild(missionsContainer);
        renderMissions(missionsContainer);
    }
    ...
    if (user && user.empresa) {
        ...
        const empresaLinkA = document.createElement('a');
        empresaLinkA.href = `empresa_local.html?empresa=${encodeURIComponent(user.empresa)}`;
        empresaLinkA.style.cssText = 'color:#00ff88;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:1px;';
        empresaLinkA.textContent = 'VER MINHA EMPRESA: ' + user.empresa.toUpperCase();
        empresaLink.appendChild(empresaLinkA);
        ...
        linkA.href = `perfil_local.html?motorista=${encodeURIComponent(user.nome)}`;
        linkA.style.cssText = 'color:#ffaa00;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:1px;';
        linkA.textContent = 'VER MEU PERFIL: ' + user.nome.toUpperCase();
...
function startPolling() {
    ...
    const interval = Math.max(config.updateIntervalMs || 100, 300); // minimum 300ms throttle
...
async function carregarEventoBanner() {
    ...
    if (getEventoEncerradoId() === String(evento.id)) {
        container.style.display = 'none';
        container.innerHTML = '';
        return;
    }
    ...
    eventoRefreshTimer = setTimeout(() => carregarEventoBanner(), 30000);
}
...
function renderMissions(container) {
    const missions = getMissionsEligible();
    ...
    container.innerHTML = `
        <div style="margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
                ...
                <span style="font-size:10px;font-weight:700;letter-spacing:2px;color:var(--accent-gold, #f5c842);text-transform:uppercase;">Missões Diárias</span>
                <span style="font-size:10px;color:#555;margin-left:auto;">Reset em <span id="missions-countdown">${formatCountdown(getMissionResetTime())}</span></span>
            </div>
            <div class="missions-container" id="missions-grid">
                ${missions.map(m => {
                    const pct = Math.min(Math.round((m.progress / m.target) * 100), 100);
                        ...
                        <div class="mission-card${done ? ' completed' : ''}">
                            <div class="mission-header">
                                ...
                                <div class="mission-info">
                                    ...
                                    <div class="mission-desc">${escapeHTML(m.desc.replace('{target}', m.target))}</div>
                                    <div class="mission-reward">+${m.reward} gold</div>
                                </div>
                                ${done ? '<span style="font-size:20px;">✅</span>' : ''}
                            </div>
                            <div class="mission-progress">
                                <div class="mission-bar">
                                    <div class="mission-fill ${done ? 'complete' : 'partial'}" style="width:${pct}%"></div>
                                </div>
                            ...
                            ${done && !m.claimed
                                ? `<button class="mission-claim ready" onclick="this.disabled=true;if(claimMissionReward('${m.id}')){this.textContent='✅ RECEBIDO';this.className='mission-claim claimed';showToast('+${m.reward} gold recebido!','success')}else{this.textContent='ERRO';this.style.background='#ff4444'}">RECEBER RECOMPENSA</button>`
                                : done && m.claimed
                                ? `<button class="mission-claim claimed" disabled>✅ RECEBIDO</button>`
                                : ''}
                        </div>
                    `;
                }).join('')}
    ...
    setInterval(() => {
        ...
        if (el) el.textContent = formatCountdown(getMissionResetTime());
    }, 1000);
    // Listen for gold earnings
    window.addEventListener('cargo-gold-earned', () => {
        showToast('Gold atualizado!', 'success');
    });
}

async function init() {
    showLoading('app', 'Inicializando dashboard...');
    await loadConfig();
    startPolling();
}
