...
function getJobHash(cargo, origem, destino, km) {
    const raw = (cargo || '') + '|' + (origem || '') + '|' + (destino || '') + '|' + (km || 0);
    let hash = 0;
...
try {
    ...
    if (saved) {
        ...
        autoRecorder.isRecording = parsed.isRecording || false;
    }
} catch (e) {}
...
function processAutoRecord(data) {
    ...
    if (!autoRecorder.isRecording && autoRecorder.lastJobActive && !jobActive && autoRecorder.lastCargo) {
        ...
        const hash = getJobHash(cargo, origin, destination, distance);
        const recordedHash = localStorage.getItem('cargo_last_job_hash');
        if (recordedHash === hash) {
            console.log('[AUTO-RECORD] Job ja registrado por outra aba, ignorando');
        ...
        autoRecordTrip(cargo, origin, destination, distance, income, cargoId, hash);
    }
...
async function autoRecordTrip(cargo, origem, destino, km, income, cargoId, jobHash) {
    const user = getAuthUser();
    if (!user) {
    ...
    try {
        ...
        if (res) {
            ...
            if (result && result.ok) {
                ...
                console.log('[AUTO-RECORD] Servidor ignorou duplicata');
                showAutoNotif(`Viagem ${km}km ja registrada (duplicata ignorada)`, '#ffaa00');
                autoRecorder.lastCargo = '';
                autoRecorder.lastCargoId = '';
                autoRecorder.lastOrigin = '';
                autoRecorder.lastDestination = '';
...
document.addEventListener('visibilitychange', async () => {
    if (document.hidden) {
        ...
        if (!autoRecorder.isRecording && lastVisibleJobActive && !jobActive && autoRecorder.lastCargo) {
            ...
            const hash = getJobHash(autoRecorder.lastCargo, autoRecorder.lastOrigin, autoRecorder.lastDestination, autoRecorder.lastDistance);
            const recordedHash = localStorage.getItem('cargo_last_job_hash');
            if (recordedHash === hash) {
                console.log('[AUTO-RECORD] Job ja registrado por outra aba, ignorando');
                autoRecorder.lastCargo = '';
            ...
            autoRecorder.isRecording = true;
            autoRecordTrip(autoRecorder.lastCargo, autoRecorder.lastOrigin, autoRecorder.lastDestination, autoRecorder.lastDistance, autoRecorder.lastIncome, autoRecorder.lastCargoId, hash);
        } else {
            processAutoRecord(data);
        }
    }
