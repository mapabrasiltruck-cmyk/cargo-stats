...
let UPLOADS_DIR = process.env.CARGOSTATS_UPLOADS_PATH || path.join(__dirname, 'uploads');
if (UPLOADS_DIR && !path.isAbsolute(UPLOADS_DIR)) {
    UPLOADS_DIR = path.resolve(__dirname, UPLOADS_DIR);
}
const MAX_SIZE = 2 * 1024 * 1024;
const ALLOWED_EXTENSIONS = new Set(['.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg', '.ico']);

const ENV_UPLOADS = process.env.CARGOSTATS_UPLOADS_PATH;
if (ENV_UPLOADS) {
    const envPath = path.isAbsolute(ENV_UPLOADS) ? ENV_UPLOADS : path.resolve(__dirname, ENV_UPLOADS);
    const oldDir = path.join(__dirname, 'uploads');
    if (!fs.existsSync(envPath) && fs.existsSync(oldDir)) {
        try {
            fs.mkdirSync(envPath, { recursive: true });
            const entries = fs.readdirSync(oldDir);
            for (const entry of entries) {
                const src = path.join(oldDir, entry);
                const dest = path.join(envPath, entry);
                if (fs.statSync(src).isFile() && !fs.existsSync(dest)) {
                    fs.copyFileSync(src, dest);
                }
            }
            console.log('[UPLOAD] Migrados arquivos existentes para:', envPath);
        } catch (e) {
