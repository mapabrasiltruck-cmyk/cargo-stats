...
function verificarViagemDuplicada(motorista, origem, destino, km) {
    const db = getDB();
    ...
    const dup = db.prepare(`
        ...
        LIMIT 1
    `).get(motorista, hoje, origem || '', destino || '', km || 0);
    return !!dup;
}
...
function inserirViagem(motorista, empresa, data, origem, destino, km, pontuacao, categoriaCarga) {
    const db = getDB();
    const cat = categoriaCarga || 'geral';
    return db.prepare(`INSERT INTO viagens (motorista, empresa, data, origem, destino, km, pontuacao, categoria_carga) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(motorista, empresa, data, origem, destino, km || 0, pontuacao || 0, cat);
}

function adicionarViagemComCategoria(motorista, empresa, data, origem, destino, km, pontuacao, categoriaCarga) {
    const result = inserirViagem(motorista, empresa, data, origem, destino, km, pontuacao, categoriaCarga);
    recalcularEmpresa(empresa);
    return result;
}

function criarViagemCompleta(motorista, empresa, data, origem, destino, km, pontuacao, categoriaCarga, eventoInfo) {
    const db = getDB();
    const cat = categoriaCarga || 'geral';
    const tx = db.transaction(() => {
