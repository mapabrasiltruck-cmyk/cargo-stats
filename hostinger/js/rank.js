...
function renderRanking(data) {
    ...
    list.forEach(function(item, i) {
        ...
        const esc = v => { if (v === null || v === undefined) return ''; return String(v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;'); };
        html += '<tr><td class="rank-pos' + posClass + '">' + pos + 'o</td>';
        html += '<td class="rank-name' + posClass + '">' + esc(item.nome || '-') + '</td>';
        ...
        if (currentTab === 'motoristas') {
            html += '<td class="rank-stat">' + esc(item.empresa || 'Lobo Solitario') + '</td>';
        }
        html += '</tr>';
    });
