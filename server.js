...
const {
    ...
    getRankingConquistas, recalcEmpresas,
    ...
    criarMotorista, atualizarMotorista, deletarMotorista, adicionarViagemComCategoria, limparDadosAntigos,
...
// Auto-seed: create admin account if no users exist
const db = getDB();
...
const server = http.createServer(async (req, res) => {
    ...
    if (urlPath === '/api/viagens' && req.method === 'POST') {
        ...
        const tripResult = adicionarViagemComCategoria(nomeMotorista, empresaMotorista, data, origem || '', destino || '', km || 0, pontuacao || 0, cat);

        const evento = getEventoAtivo();
        let bonusInfo = null;
        if (evento) {
            const progResult = atualizarProgressoEvento(evento.id, nomeMotorista, empresaMotorista, { km: km || 0, pontuacao: pontuacao || 0, categoria_carga: cat, destino: destino || '' });
            if (progResult && progResult.metaAtingida && progResult.bonus_pontos > 0) {
                adicionarBonusViagem(tripResult.lastInsertRowid, progResult.bonus_pontos);
                recalcEmpresas();
    ...
    if (urlPath === '/api/viagens/auto' && req.method === 'POST') {
        ...
        const tripResult = adicionarViagemComCategoria(motorista, empresaAuto, hoje, origem || '', destino || '', km || 0, pontuacao || 0, cat);

        const evento = getEventoAtivo();
        let bonusInfo = null;
        if (evento) {
            const progResult = atualizarProgressoEvento(evento.id, motorista, empresaAuto, { km: km || 0, pontuacao: pontuacao || 0, categoria_carga: cat, destino: destino || '' });
            if (progResult && progResult.metaAtingida && progResult.bonus_pontos > 0) {
                adicionarBonusViagem(tripResult.lastInsertRowid, progResult.bonus_pontos);
                recalcEmpresas();
                bonusInfo = { bonus_pontos: progResult.bonus_pontos, bonus_km: progResult.bonus_km || 0 };
